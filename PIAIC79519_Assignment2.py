...
Original file is located at
    https://colab.research.google.com/drive/1J9P__im3JeL1hOAih5e4zCxPOhFAUBAC
"""
...
def function7():
    ...
    return arr,arr.dtype #write your code here
...
def function11():
    ...
    odd_values = (arr%2 == 1)  
    arr[odd_values] = -1#write your code here 
...
def function12():
    ...
    rpt = np.repeat(arr,3)
    ans = np.tile(arr,3)
    ans = np.hstack((rpt,ans)) #write your code here 
...
def function18():
    ...
    freq = np.zeros(  len(piaic)  )

    for i in range( len(piaic) ):
        mark = np.where( students==piaic[i] )
        freq[i] = len(mark[0])
    x = int(np.sum(freq)) # Write you code Here
    return x
...
def function19():
    ...
    W =   X.T.copy()# Write your code here 
...
def fucntion20():
    ...
    def abc(x):
        return x*2+3-2
    result = map(abc,x)
    return list(result)  #Write your Code here
